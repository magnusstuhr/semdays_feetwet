...
<xsl:template match="result" mode="script">
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#openSearch')
				.toggle(function () {
					$(this).text('Skjul søkeskjema');
					$('#search').slideDown('slow');
				}, function () {
					$(this).text('Vis søkeskjema');
					$('#search').slideUp('slow');
				});
...
<xsl:template match="viewingResult | selectionResult" mode="footer">
	<div>
		...
		<h2>
			<xsl:choose>
				<xsl:when test="self::viewingResult">Visning</xsl:when>
				<xsl:otherwise>Utvelgelse</xsl:otherwise>
			</xsl:choose>
		</h2>
		...
		<p>
			<xsl:text>Dette er SPARQL-spørringen som ble generert for å </xsl:text>
			<xsl:choose>
				<xsl:when test="self::viewingResult">
					<xsl:text>hente dataene som er synlige for</xsl:text>
					<xsl:choose>
						<xsl:when test="/result/items"> hver ressurs.</xsl:when>
						<xsl:otherwise> den valgte ressursen.</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>identifisere ressursen(e) som vises på siden.</xsl:otherwise>
			</xsl:choose>
			
			<xsl:text> Du kan modifisere spørringen her og kjøre den på nytt mot </xsl:text>
			<a href="{$formToUse}">SPARQL-endepunktet</a>
			<xsl:text> som brukes.</xsl:text>
		</p>
		<form action="{$endPointToUse}" method="post">
			<xsl:apply-templates select="query" mode="footer" />
			<p>
				<button type="submit">Kjør spørring</button>
			</p>
		</form>
	</div>
</xsl:template>
			...
			<xsl:for-each select="format/item">
				...
				<li>
					<xsl:if test="position() = 1">
						<xsl:attribute name="class">første</xsl:attribute>
					</xsl:if>
					<xsl:if test="position() = last()">
						<xsl:attribute name="class">siste</xsl:attribute>
					</xsl:if>
				...
				<xsl:call-template name="createInfo">
					<xsl:with-param name="text">
						...
						<xsl:if test="$searchMap">
							...
							<img src="{$activeImageBase}/Search.png" alt="søk" />
...
<xsl:template match="result" mode="summary">
	<xsl:if test="count(items/item) > 1">
		<section class="summary">
			<h1>På denne siden</h1>
			<xsl:call-template name="createInfo">
				<xsl:with-param name="text">Linker til resurssene bekskrevet på denne siden og til forrige/neste side med resultater.</xsl:with-param>
			</xsl:call-template>
			<ul>
				<xsl:if test="prev">
					<li>
...
<xsl:template match="result" mode="viewnav">
	...
	<section class="view">
		<h1>Synlige egenskaper</h1>
		<xsl:call-template name="createInfo">
			<xsl:with-param name="text">
				<xsl:text>Velg hvilken informasjon du vil se om hver ressurs. </xsl:text>
				<xsl:text>Stjernemerkede egenskaper er alltid synlige. </xsl:text> 
				<xsl:text>Stjernemarkér en egenskap ved å klikke på </xsl:text>
				<img src="{$inactiveImageBase}/Star.png" alt="star this property" />
				<xsl:text>-ikonet. De stjernemerkede egenskapene har et </xsl:text>
				<img src="{$activeImageBase}/Star.png" alt="unstar this property" />
				<xsl:text>-ikon; klikk på det for å avmerke egenskapen.</xsl:text>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:if test="$properties != ''">
			<p class="reset">
				<a title="avmerk alle egenskaper">
					<xsl:attribute name="href">
						<xsl:call-template name="substituteParam">
							<xsl:with-param name="uri" select="/result/@href" />
							<xsl:with-param name="param" select="'_properties'" />
...
<xsl:template match="result" mode="selectedProperties">
	...
	<li class="selected">
		<a rel="nofollow" title="remove this property">
			...
			<img src="{$activeImageBase}/Star.png" alt="avmerk denne egenskapen" />
		</a>
		<xsl:text> </xsl:text>
		<xsl:choose>
			<xsl:when test="$isLabelParam = 'true'">
...
<xsl:template match="*" mode="properties">
	...
	<xsl:if test="not(contains(concat(',', $properties, ','), concat(',', $name, ',')))">
		<li>
			<a rel="nofollow" title="vis denne egenskapen alltid">
				<xsl:attribute name="href">
					<xsl:call-template name="substituteParam">
						<xsl:with-param name="uri" select="/result/@href" />
						<xsl:with-param name="param" select="'_properties'" />
...
<xsl:template match="result" mode="sizenav">
	<section class="size">
		<h1>Ressurser per side</h1>
		<xsl:call-template name="createInfo">
			<xsl:with-param name="text">Velg hvor mange resursser du ønsker å vise per side. Siden vil ta lenger tid å laste jo flere ressurser per side som velges.</xsl:with-param>
		</xsl:call-template>
		<ul>
			<li>
...
<xsl:template match="result" mode="pageSize">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<a rel="nofollow" title="vis {$size} ressurser per side">
				<xsl:attribute name="href">
					<xsl:call-template name="substituteParam">
						<xsl:with-param name="uri">
							<xsl:apply-templates select="/result" mode="searchURI" />
...
<xsl:template match="result" mode="sortnav">
	...
	<section class="sort">
		...
		<xsl:call-template name="createInfo">
			<xsl:with-param name="text">
				<xsl:text>Egenskaper du kan sortere etter. Klikk på </xsl:text>
				<img src="{$inactiveImageBase}/Arrow3 Up.png" alt="sort in ascending order" />
				<xsl:text> for å sortere i stigende rekkefølge og </xsl:text>
				<img src="{$inactiveImageBase}/Arrow3 Down.png" alt="sort in descending order" />
				<xsl:text> for å sortere etter synkende rekkefølge. Klikk på </xsl:text>
				<img src="{$activeImageBase}/Cancel.png" alt="remove this sort" />
				<xsl:text> for å fjerne en sortering og </xsl:text>
				...
				<xsl:text> for å reversere valgt sorteringsrekkefølge. </xsl:text>
				<xsl:text>Klikk på </xsl:text>
				<img src="{$activeImageBase}/Back.png" alt="remove all sorting" />
				<xsl:text> ikonet for å fjerne all sortering. </xsl:text>
				<xsl:text>Merk at sortering kan gjøre lastingen av sidene betydelig tregere.</xsl:text>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:if test="$current != ''">
			<p class="reset">
				<a title="fjern sortering">
					<xsl:attribute name="href">
						<xsl:call-template name="substituteParam">
					...
					<li class="selected">
						...
						<xsl:choose>
							...
							<xsl:when test="starts-with($orderBy, 'desc')">
								<a rel="nofollow" title="sortér etter stigende rekkefølge">
									<xsl:attribute name="href">
										<xsl:call-template name="substituteParam">
											<xsl:with-param name="uri" select="$baseURI" />
											<xsl:with-param name="param" select="'_orderBy'" />
							...
							<xsl:when test="starts-with($orderBy, 'asc')">
								<a rel="nofollow" title="sortér etter synkende rekkefølge">
									<xsl:attribute name="href">
										<xsl:call-template name="substituteParam">
											<xsl:with-param name="uri" select="$baseURI" />
											<xsl:with-param name="param" select="'_orderBy'" />
...
<xsl:template match="result" mode="selectedSorts">
	...
	<li class="selected">
		<a rel="nofollow" title="fjern sortering">
			<xsl:attribute name="href">
				<xsl:call-template name="substituteParam">
					<xsl:with-param name="uri" select="$uri" />
					<xsl:with-param name="param" select="'_sort'" />
			...
			<img src="{$activeImageBase}/Cancel.png" alt="fjern soirtering" />
		</a>
		<a rel="nofollow">
			<xsl:attribute name="href">
				<xsl:call-template name="substituteParam">
			...
			<xsl:attribute name="title">
				<xsl:choose>
					<xsl:when test="starts-with($sort, '-')">sortér i stigende rekkefølge</xsl:when>
					<xsl:otherwise>sortér i synkende rekkefølge</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<xsl:choose>
				<xsl:when test="starts-with($sort, '-')">
...
<xsl:template match="*" mode="sort">
	...
	<xsl:if test="not(contains(concat(',', $current, ','), concat(',', $name, ',')) or contains(concat(',', $current, ','), concat(',-', $name, ',')))">
		...
		<li>
			<a rel="nofollow" href="{$ascending}" title="sortér i stigende rekkefølge">
				<img src="{$inactiveImageBase}/Arrow3 Up.png" alt="sort in ascending order" />
			</a>
			<a rel="nofollow" title="sortér i synkende rekkefølge">
				<xsl:attribute name="href">
					<xsl:call-template name="substituteParam">
						<xsl:with-param name="uri" select="$uri" />
						<xsl:with-param name="param" select="'_sort'" />
				...
				<img src="{$inactiveImageBase}/Arrow3 Down.png" alt="sortér i synkende rekkefølge" />
			</a>
			<xsl:text> </xsl:text>
			<a rel="nofollow" href="{$ascending}" title="sortér in stigende rekkefølge">
				<xsl:apply-templates select="." mode="contextLabel" />
			</a>
		</li>
	</xsl:if>
...
<xsl:template match="first | prev | next | last" mode="nav">
	<a href="{@href}" title="gå til {name()} side">
		<xsl:choose>
...
<xsl:template match="format/item" mode="nav">
	...
	<a href="{@href}" type="{format/label}" rel="alternate" title="vis i {$name}-format">
...
<xsl:template match="*" mode="filter">
	...
	<xsl:choose>
		...
		<xsl:when test="$datatype = 'integer' or $datatype = 'decimal' or $datatype = 'float' or $datatype = 'int' or $datatype = 'date' or $datatype = 'dateTime' or $datatype = 'time'">
			...
			<xsl:choose>
				<xsl:when test="$max = $value">
					<a rel="nofollow" title="fjern maksimumsverdifiltrering">
						<xsl:attribute name="href">
							<xsl:call-template name="substituteParam">
								<xsl:with-param name="uri">
									<xsl:apply-templates select="/result" mode="searchURI" />
				...
				<xsl:otherwise>
					<a rel="nofollow" title="se verdier med verdi mindre enn {$value}">
						<xsl:attribute name="href">
							<xsl:call-template name="substituteParam">
								<xsl:with-param name="uri">
									<xsl:apply-templates select="/result" mode="searchURI" />
			...
			<a rel="nofollow" title="finn andre ressurser med samme verdi for denne egenskapen">
				<xsl:attribute name="href">
					<xsl:call-template name="substituteParam">
						<xsl:with-param name="uri">
							<xsl:apply-templates select="/result" mode="searchURI" />
			...
			<xsl:choose>
				<xsl:when test="$min = $value">
					<a rel="nofollow" title="fjern minimumsverdifilter">
						<xsl:attribute name="href">
							<xsl:call-template name="substituteParam">
								<xsl:with-param name="uri">
									<xsl:apply-templates select="/result" mode="searchURI" />
				...
				<xsl:otherwise>
					<a rel="nofollow" title="større enn {$value}">
						<xsl:attribute name="href">
							<xsl:call-template name="substituteParam">
								<xsl:with-param name="uri">
									<xsl:apply-templates select="/result" mode="searchURI" />
		...
		<xsl:otherwise>
			<a rel="nofollow" title="se ressurser med samme verdi">
				<xsl:attribute name="href">
					<xsl:call-template name="substituteParam">
						<xsl:with-param name="uri">
							<xsl:apply-templates select="/result" mode="searchURI" />
